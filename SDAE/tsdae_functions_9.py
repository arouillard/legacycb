...
def main(d):
    ...
    structured_noise_mask = tf.to_float(tf.random_uniform((tf.shape(x)[tf.to_int32(zero_)], tf.to_int32(one_))) <= noise_prob)*tf.abs(tf.to_float(tf.random_uniform((tf.shape(x)[tf.to_int32(zero_)], tf.to_int32(one_))) <= half_) - is_bernoulli_marginal)
