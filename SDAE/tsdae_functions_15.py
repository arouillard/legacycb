...
def main(d):
    ...
    one = 1.
#    true = True
    num_bernoulli_rows = np.float64(np.median(np.sum(np.isfinite(dataset['train'].matrix[:,dataset['train'].columnmeta['likelihood'] == 'bernoulli',]), 0)))
    a_sigm = 2.0/num_bernoulli_rows # this controls how extreme the ratio of positive to negative weight can be. 2.0/num_bernoulli_rows translates to a ratio of 2.7. 10.0/num_bernoulli_rows translates to a ratio of 148.
    b_sigm = -0.5*num_bernoulli_rows
    poscounts = np.nansum(dataset['train'].matrix, 0, keepdims=True)
    posweights = 1/(1 + np.exp(-a_sigm*(poscounts + b_sigm)))
    negcounts = np.sum(np.isfinite(dataset['train'].matrix), 0, keepdims=True) - np.nansum(dataset['train'].matrix, 0, keepdims=True)
    negweights = 1/(1 + np.exp(-a_sigm*(negcounts + b_sigm)))
    weightsums = posweights*poscounts + negweights*negcounts
    posweights /= weightsums
    negweights /= weightsums
...
#    negweights[:,dataset['train'].columnmeta['likelihood'] != 'bernoulli'] = 1/np.sum(~np.isnan(dataset['train'].matrix[:,dataset['train'].columnmeta['likelihood'] != 'bernoulli']), 0, keepdims=True)
    ...
    nanvalue = -666.666 # must be negative
    for partition in partitions:
...
#    loss_weights = is_complete_row*(pos_weights*is_positive + neg_weights*(one_-is_positive))*(one_-is_missing)*dataset_weights # missing values won't be included in loss calculation
#    loss_weights = loss_weights/tf.reduce_mean(loss_weights)
#    loss_weights = is_complete_row*(one_-is_missing)
#    loss_weights = (one_-is_missing)
#    loss_weights = is_complete_row*(one_-is_missing)*dataset_weights
#    loss_weights = (one_-is_missing)*dataset_weights
#    loss_weights = loss_weights/tf.reduce_mean(loss_weights)
...
#    structured_noise_mask = is_complete_row*tf.to_float(tf.random_uniform((tf.shape(x)[tf.to_int32(zero_)], tf.to_int32(one_))) <= structured_noise_prob)*tf.abs(tf.to_float(tf.random_uniform((tf.shape(x)[tf.to_int32(zero_)], tf.to_int32(one_))) > bernoulli_weight) - is_bernoulli_marginal)
...
#                datasetIO.save_datamatrix('{0}/{1}_embedding_preactivation_layer{2!s}_finetuning{3!s}.txt.gz'.format(d['output_path'], partition, d['current_hidden_layer'], d['current_finetuning_run']), embed_preactivation[partition])
    ...
    dataset['all'].matrix[:,dataset['all'].columnmeta['likelihood'] == 'bernoulli'] = np.nan       
    recon = {}
    embed = {}
    error = {}
    embed_preactivation = {}
    for partition in ['all']:
        if np.isnan(dataset[partition].matrix).any():
            ...
            recon[partition].matrix /= 25
            embed[partition].matrix /= 25
            error[partition] /= 25
            if d['current_apply_activation_to_embedding']:
                embed_preactivation[partition].matrix /= 25
        else:
            if d['use_batchnorm']:
                recon[partition], embed[partition], error[partition] = tsdae_apply_functions.encode_and_decode(dataset[partition], W, Be, Bd, activation_function['np'], d['current_apply_activation_to_embedding'], d['apply_activation_to_output'], dataset['train'].columnmeta['likelihood'] == 'bernoulli', return_embedding=True, return_reconstruction_error=True, bn_encode_variables=batchnorm_encode_variables, bn_decode_variables=batchnorm_decode_variables)
                if d['current_apply_activation_to_embedding']:
...
#                datasetIO.save_datamatrix('{0}/{1}_embedding_preactivation_layer{2!s}_finetuning{3!s}.txt.gz'.format(d['output_path'], partition, d['current_hidden_layer'], d['current_finetuning_run']), embed_preactivation[partition])
    ...
    hitmat = np.logical_and(isbernoullimarginal.astype('bool'), np.isfinite(dataset['valid'].matrix))
    print('evaluating performance by row...', flush=True)
    aurocs = []
    auprcs = []
    aps = []
    with gzip.open('{0}/performance_per_row_layer{1!s}_finetuning{2!s}.txt.gz'.format(d['output_path'], d['current_hidden_layer'], d['current_finetuning_run']), 'wt') as fw:
        fw.write('\t'.join(['label', 'auroc', 'auprc', 'ap']) + '\n')
        for i, (hm, dm, rm, label) in enumerate(zip(hitmat, dataset['valid'].matrix, recon['valid'].matrix, dataset['valid'].rowlabels)):
            if hm.any() and np.unique(dm[hm]).size == 2:
                sc = modelevaluation.get_classifier_performance_stats(dm[hm].astype('bool'), rm[hm], uP=10000, classifier_stats='all', plot_curves=False, get_priority_cutoffs=True, pp_min_frac=0.1, xx_min_frac=0.01)
                aurocs.append(sc.select('auroc',[])[0])
                auprcs.append(sc.select('auprc',[])[0])
                aps.append(sc.select('ap',[])[0])
                fw.write('\t'.join([label] + ['{0:1.5g}'.format(x) for x in [sc.select('auroc',[])[0], sc.select('auprc',[])[0], sc.select('ap',[])[0]]]) + '\n')
    print('plotting performance by row...', flush=True)
    fg, ax = plt.subplots(1, 1, figsize=(3.25,2.25))
    ax.set_position([0.55/3.25, 0.45/2.25, 2.6/3.25, 1.7/2.25])
    ax.semilogx(aps, aurocs, 'ok', alpha=0.5, markeredgewidth=0, markersize=5)
    ax.set_ylabel('AUROC', fontsize=8)
    ax.set_xlabel('ACTUAL POSITIVES', fontsize=8)
    ax.set_ylim(0, 1)
    ...
    fg.savefig('{0}/auroc_per_row_layer{1!s}_finetuning{2!s}.png'.format(d['output_path'], d['current_hidden_layer'], d['current_finetuning_run']), transparent=True, pad_inches=0, dpi=600)
    plt.close()
    fg, ax = plt.subplots(1, 1, figsize=(3.25,2.25))
    ax.set_position([0.55/3.25, 0.45/2.25, 2.6/3.25, 1.7/2.25])
    ax.semilogx(aps, auprcs, 'ok', alpha=0.5, markeredgewidth=0, markersize=5)
    ax.set_ylabel('AUPRC', fontsize=8)
    ax.set_xlabel('ACTUAL POSITIVES', fontsize=8)
    ax.set_ylim(0, 1)
    ax.tick_params(axis='both', which='major', left=True, right=True, bottom=True, top=False,
                   labelleft=True, labelright=False, labelbottom=True, labeltop=False, labelsize=8)
    fg.savefig('{0}/auprc_per_row_layer{1!s}_finetuning{2!s}.png'.format(d['output_path'], d['current_hidden_layer'], d['current_finetuning_run']), transparent=True, pad_inches=0, dpi=600)
    plt.close()
    print('evaluating performance by column...', flush=True)
    aurocs = []
    auprcs = []
    aps = []
    with gzip.open('{0}/performance_per_col_layer{1!s}_finetuning{2!s}.txt.gz'.format(d['output_path'], d['current_hidden_layer'], d['current_finetuning_run']), 'wt') as fw:
        fw.write('\t'.join(['label', 'auroc', 'auprc', 'ap']) + '\n')
        for i, (hm, dm, rm, label) in enumerate(zip(hitmat.T, dataset['valid'].matrix.T, recon['valid'].matrix.T, dataset['valid'].columnlabels)):
            if hm.any() and np.unique(dm[hm]).size == 2:
                sc = modelevaluation.get_classifier_performance_stats(dm[hm].astype('bool'), rm[hm], uP=10000, classifier_stats='all', plot_curves=False, get_priority_cutoffs=True, pp_min_frac=0.1, xx_min_frac=0.01)
                aurocs.append(sc.select('auroc',[])[0])
                auprcs.append(sc.select('auprc',[])[0])
                aps.append(sc.select('ap',[])[0])
                fw.write('\t'.join([label] + ['{0:1.5g}'.format(x) for x in [sc.select('auroc',[])[0], sc.select('auprc',[])[0], sc.select('ap',[])[0]]]) + '\n')
    print('plotting performance by column...', flush=True)
    fg, ax = plt.subplots(1, 1, figsize=(3.25,2.25))
    ax.set_position([0.55/3.25, 0.45/2.25, 2.6/3.25, 1.7/2.25])
    ax.semilogx(aps, aurocs, 'ok', alpha=0.5, markeredgewidth=0, markersize=5)
    ax.set_ylabel('AUROC', fontsize=8)
    ...
    ax.tick_params(axis='both', which='major', left=True, right=True, bottom=True, top=False,
                   labelleft=True, labelright=False, labelbottom=True, labeltop=False, labelsize=8)
    fg.savefig('{0}/auroc_per_col_layer{1!s}_finetuning{2!s}.png'.format(d['output_path'], d['current_hidden_layer'], d['current_finetuning_run']), transparent=True, pad_inches=0, dpi=600)
    plt.close()
    fg, ax = plt.subplots(1, 1, figsize=(3.25,2.25))
    ax.set_position([0.55/3.25, 0.45/2.25, 2.6/3.25, 1.7/2.25])
    ax.semilogx(aps, auprcs, 'ok', alpha=0.5, markeredgewidth=0, markersize=5)
    ...
    ax.tick_params(axis='both', which='major', left=True, right=True, bottom=True, top=False,
                   labelleft=True, labelright=False, labelbottom=True, labeltop=False, labelsize=8)
    fg.savefig('{0}/auprc_per_col_layer{1!s}_finetuning{2!s}.png'.format(d['output_path'], d['current_hidden_layer'], d['current_finetuning_run']), transparent=True, pad_inches=0, dpi=600)
    plt.close()
