...
def main(d):
    ...
    print('weight matrices:{0!s}'.format(len(W)), flush=True) #########################
    print('variables to train:{0!s}'.format(len(variables_to_train)), flush=True) #####################
    ...
    if os.path.exists(d['previous_variables_path']):
        ...
        global_step, W, bencode, bdecode = update_variables((global_step, W, bencode, bdecode), d['previous_variables_path'], d['fix_or_init'], d['include_global_step'])
        variables_to_train = tf.trainable_variables() ##########################
        print('weight matrices:{0!s}'.format(len(W)), flush=True) #########################
        print('variables to train:{0!s}'.format(len(variables_to_train)), flush=True) #####################
    elif d['current_hidden_layer'] > 1:
    ...
    with tf.Session(config=session_config) as sess:
            ...
            for epoch in range(d['current_epochs']):
                ...
                for batch in range(d['batches']):
                    ...
                    if training_step == reporting_steps[i]:
                        ...
                        if training_step == 800 and d['current_hidden_layer'] == 3 and d['current_finetuning_run'] == 1: #################
                            stopearly = True ################
                            
                        # save current weights
                        if training_step >= d['startsavingstep'] and training_step < reporting_steps[-1]:
        ...
        if stopearly:
            ...
            rollback_variables_path = '{0}/intermediate_variables_layer{1!s}_finetuning{2!s}_step{3!s}.pickle'.format(d['output_path'], d['current_hidden_layer'], d['current_finetuning_run'], max([reporting_steps[i-d['overfitting_score_max']], d['startsavingstep']]))
        ...
        with open('{0}/variables_layer{1!s}_finetuning{2!s}.pickle'.format(d['output_path'], d['current_hidden_layer'], d['current_finetuning_run']), 'wb') as fw:
            pickle.dump((sess.run(global_step), sess.run(W), sess.run(bencode), sess.run(bdecode)), fw)
        ...
        is_recon[train.shape[0]:train.shape[0]+d['reconstruction_rows']*d['reconstruction_cols']] = True
