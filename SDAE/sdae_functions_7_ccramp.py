...
import sdae_apply_functions_7_ccramp as sdae_apply_functions # if apply_activation_to_embedding=True, SOFTMAX is used for embedding
import datasetIO
...
def create_autoencoder(x, activation, apply_activation_to_output, apply_activation_to_embedding, use_softmax, use_batchnorm, is_training, W, bencode, bdecode):
        ...
        if use_batchnorm:
            ...
            if i == len(W)-1:
                if apply_activation_to_embedding:
                    if use_softmax:
                        h.append(tf.nn.softmax(tf.matmul(h[i], w) + be))
                    else:
...
def main(d):
    ...
    elif d['activation_function'] == 'softmax':
        activation_function = {'tf':tf.nn.softmax, 'np':sdae_apply_functions.softmax}
    ...
    # get labels
    labels = dataset['train'].rowmeta['general_tissue'].copy()
    ulabels = np.unique(labels) # 28
    ...
    for label in ulabels:
        hit = labels == label
        label_hitbatchids[label] = [np.append(hit, np.zeros(dataset['valid'].shape[0], dtype='bool')), create_batch_ids(hit.sum(), d['batch_size']/d['train_examples']*hit.sum())]
    
    # get ramp decay
    rampdecay = d['last_layer_epochs']*d['batches']/10.0
    ...
    pair_mask = tf.placeholder(tf.bool, [dataset['train'].shape[0]+dataset['valid'].shape[0]])
    ...
    ramp_decay_initializer = tf.placeholder(tf.float32, [])
    ...
    ramp_decay = tf.Variable(ramp_decay_initializer, trainable=False, collections=[])
    ...
    y = tf.boolean_mask(training_and_validation_data, pair_mask)
...
#        noise = tf.to_float(noise_distribution(tf.shape(x), minval=0, maxval=1) > marginal_probabilities) # 1 - marginal
    ...
    if d['activation_function'] == 'sigmoid' and d['apply_activation_to_output']: # because need the preactivation to compute the cross entropy loss
        h, hhat, xhat = create_autoencoder(xnoisy, activation_function['tf'], False, d['current_apply_activation_to_embedding'], use_softmax, d['use_batchnorm'], training, W, bencode, bdecode)
    else:
        h, hhat, xhat = create_autoencoder(xnoisy, activation_function['tf'], d['apply_activation_to_output'], d['current_apply_activation_to_embedding'], use_softmax, d['use_batchnorm'], training, W, bencode, bdecode)
...
#        loss = tf.reduce_mean(tf.squared_difference(y, xhat)) # squared error loss
#        loss = 0.5*tf.reduce_mean(tf.squared_difference(x, xhat)) + 0.5*tf.reduce_mean(tf.squared_difference(y, xhat)) # squared error loss
        recon_weight = 0.5*tf.exp(-1.0*tf.to_float(global_step)/ramp_decay)
        loss = recon_weight*tf.reduce_mean(tf.squared_difference(x, xhat)) + (1.0 - recon_weight)*tf.reduce_mean(tf.squared_difference(y, xhat)) # squared error loss
...
#    bottleneck_preactivation = tf.matmul(h[-2], W[-1]) + bencode[-1]
    ...
    with tf.Session(config=session_config) as sess:
        ...
        sess.run(ramp_decay.initializer, feed_dict={ramp_decay_initializer: rampdecay})
        validation_id = -1
        batch_and_validation_ids = np.full(dataset['train'].shape[0]+dataset['valid'].shape[0], validation_id, dtype=batch_ids.dtype)
        pairing_ids = np.full(dataset['train'].shape[0]+dataset['valid'].shape[0], validation_id, dtype=batch_ids.dtype)
        ...
        with open('{0}/log_layer{1!s}_finetuning{2!s}.txt'.format(d['output_path'], d['current_hidden_layer'], d['current_finetuning_run']), mode='wt', buffering=1) as fl:
            ...
            for epoch in range(d['current_epochs']):
                ...
                for label, (hit, batchids) in label_hitbatchids.items():
                    np.random.shuffle(batchids)
                    batch_and_validation_ids[hit] = batchids
                    np.random.shuffle(batchids)
                    pairing_ids[hit] = batchids
                ...
                for batch in range(d['batches']):
                    ...
                    paired = pairing_ids == batch
                    
                    # update weights
                    sess.run(train_ops, feed_dict={training:True, selection_mask:selected, pair_mask:paired, noise_prob:d['noise_probability'], noise_stdv:d['noise_sigma']})
                    
                    # record training and validation errors
                    if training_step == reporting_steps[i]:
                        train_losses[i] = sess.run(loss, feed_dict={training:False, selection_mask:is_train, pair_mask:is_train, noise_prob:0, noise_stdv:0})
                        train_noisy_losses[i] = sess.run(loss, feed_dict={training:False, selection_mask:is_train, pair_mask:is_train, noise_prob:d['noise_probability'], noise_stdv:d['noise_sigma']})
                        valid_losses[i] = sess.run(loss, feed_dict={training:False, selection_mask:is_valid, pair_mask:is_valid, noise_prob:0, noise_stdv:0})
                        valid_noisy_losses[i] = sess.run(loss, feed_dict={training:False, selection_mask:is_valid, pair_mask:is_valid, noise_prob:d['noise_probability'], noise_stdv:d['noise_sigma']})
...
#                        recwgt = sess.run(recon_weight)
#                        print('step:{0:1.6g}, train loss:{1:1.3g}, valid loss:{2:1.3g}, train noisy loss:{3:1.3g}, valid noisy loss:{4:1.3g}, recon weight:{5:1.3g}, time:{6:1.6g}'.format(reporting_steps[i], train_losses[i], valid_losses[i], train_noisy_losses[i], valid_noisy_losses[i], recwgt, time.time() - starttime), flush=True)
