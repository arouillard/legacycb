...
def encode(dm, W, Be, activation, apply_activation_to_embedding=False, bn_variables=None):
    mat = dm.matrix.copy()
...
def decode(em, W, Bd, activation, apply_activation_to_output=False, output_activation_mask=[], bn_variables=None):
    mat = em.matrix.copy()
    if bn_variables == None:
        for i, (w, b) in enumerate(zip(W[::-1], Bd[::-1])):
            if i+1 < len(W):
                ...
                mat = mat.dot(w.T) + b
                mat[:,output_activation_mask] = activation(mat[:,output_activation_mask])
            else:
                mat = mat.dot(w.T) + b
    else:
        gammas, betas, moving_means, moving_variances = bn_variables
...
def encode_and_decode(dm, W, Be, Bd, activation, apply_activation_to_embedding=False, apply_activation_to_output=False, output_activation_mask=[], return_embedding=False, return_reconstruction_error=False, bn_encode_variables=None, bn_decode_variables=None):
    mat = dm.matrix.copy()
