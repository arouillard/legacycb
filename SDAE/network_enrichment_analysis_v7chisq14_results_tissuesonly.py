...
#        tobediscarded = np.logical_or(tobediscarded, np.in1d(atb_cst['is_significant'].rowlabels, list(dataset_feature_tissuespecific[datasetabbrev].keys())))
    ...
    print(atb_cst['test_statistic_values'].matrix.max(1).min())
    ...
    # save snlp layer
    with open('univariate_feature_importance_{0}/{1}_{2}_significant.pickle'.format(analysis_version, datasetabbrev, 'snlp'), mode='wb') as fw:
        pickle.dump(atb_cst['snlp'], fw)
    # plot results
