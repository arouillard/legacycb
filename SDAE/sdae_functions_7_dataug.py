...
def main(d):
    ...
    pair_weight = tf.random_uniform([tf.reduce_sum(tf.to_int32(selection_mask)), 1])
    x = (1-pair_weight)*tf.boolean_mask(training_and_validation_data, selection_mask) + pair_weight*tf.boolean_mask(training_and_validation_data, pair_mask)
    ...
    with tf.Session(config=session_config) as sess:
        ...
        with open('{0}/log_layer{1!s}_finetuning{2!s}.txt'.format(d['output_path'], d['current_hidden_layer'], d['current_finetuning_run']), mode='wt', buffering=1) as fl:
            ...
            for epoch in range(d['current_epochs']):
                ...
                np.random.shuffle(batch_ids)
                pairing_ids[is_train] = batch_ids
