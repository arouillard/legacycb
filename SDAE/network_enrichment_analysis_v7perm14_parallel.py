...
def count_function(d_shuffle, d_obs, tail='both'):
    if tail == 'left': # how likely is it to see differences as small as observed
        ...
        return np.abs(d_shuffle) >= np.abs(d_obs)
    else:
        raise ValueError('tail must be "left", "right", or "both"')

def perm_scorer(Xt, Y, positives, negatives, diff_obs, tail, hits, perm):
    Y_shuffle = np.random.permutation(Y)
    np.random.shuffle(Y_shuffle)
    diff_shuffle = (Xt).dot(Y_shuffle)/positives - (Xt).dot(~Y_shuffle)/negatives
    hits[:,:,perm] = count_function(diff_shuffle, diff_obs, tail)

def univariate_vectorized_permtest(X, Y, numperm=10**5, tail='both', datasetabbrev=''):
    ...
    hits = np.memmap('temp/{0}hits.memmap'.format(datasetabbrev), dtype='bool', mode='w+', shape=(diff_obs.shape[0], diff_obs.shape[1], numperm))
    Parallel(n_jobs=10, verbose=10, temp_folder='temp')(delayed(perm_scorer)(Xt, Y, positives, negatives, diff_obs, tail, hits, perm) for perm in range(numperm))
    count = np.array(hits.sum(2), dtype='float64')
    del hits
    os.remove('temp/hits.memmap')
...
if __name__ == '__main__':
    ...
    # parameters
    feature_selection_test_function = univariate_vectorized_permtest
    ...
    #        if datasetabbrev != 'reactome': # not in {'kegg', 'panther', 'reactome', 'wikipathways'}:
    #            continue
    ...
    #    gene_atb.rowmeta['class'][np.in1d(gene_atb.rowlabels, class_examples['positive'])] = 'positive'
    #    gene_atb.rowmeta['class'][np.in1d(gene_atb.rowlabels, class_examples['negative'])] = 'negative'
    #        # add mean and standard deviation as features
    #        Xs = np.append(gene_atb.matrix.mean(1).reshape(-1,1), gene_atb.matrix.std(1).reshape(-1,1), 1)
        Fs = np.array(['mean', 'stdv'], dtype='object')
    #        gene_atb.matrix = np.append(gene_atb.matrix.astype('float64'), Xs, axis=1)
    #        gene_atb.columnlabels = np.append(gene_atb.columnlabels, Fs)
    #        for columnmetaname in gene_atb.columnmeta:
    #            gene_atb.columnmeta[columnmetaname] = np.append(gene_atb.columnmeta[columnmetaname], np.full(Fs.size, '-666', dtype='object'))
        gene_atb.columnmeta['isrowstat'] = np.in1d(gene_atb.columnlabels, Fs)
    ...
    #        del Xs, Fs
        ...
        del commongenes
        # discard bad columns
        tobediscarded = np.logical_or((gene_atb.matrix != 0).sum(axis=0) < 3, np.isnan(gene_atb.matrix).any(axis=0))
    ...
    #        gene_atb.columnmeta['arbitrary_pvalues'] = np.zeros(gene_atb.shape[1], dtype='float64')
    #        gene_atb.columnmeta['arbitrary_pvalues'][~gene_atb.columnmeta['isrowstat']] = featureselection.univariate_chisquare(X=gene_atb.matrix[:,~gene_atb.columnmeta['isrowstat']], Y=gene_cst.matrix[:,0] < 0.2)[1]
    #        gene_atb.columnmeta['arbitrary_pvalues'][gene_atb.columnmeta['isrowstat']] = featureselection.univariate_utest(X=gene_atb.matrix[:,gene_atb.columnmeta['isrowstat']], Y=gene_cst.matrix[:,0] < 0.2)[1]
    ...
    #    atb_cst['test_statistic_values'].matrix, atb_cst['pvalues'].matrix = feature_selection_test_function(X=gene_atb.matrix, Y=gene_cst.matrix, numperm=10**5)
        atb_cst['test_statistic_values'].matrix, atb_cst['pvalues'].matrix = feature_selection_test_function(X=gene_cst.matrix, Y=gene_atb.matrix, numperm=10**5, datasetabbrev=datasetabbrev)
