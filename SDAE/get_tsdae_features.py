...
def main(visualizations_path, random_imputation_iterations=5, save_reconstructions=False, nan_datasets=[]):
    ...
    for didx, (design_path, selected_step) in enumerate(designpath_selectedstep):
        ...
        # nan datasets
        for dataset_name in nan_datasets:
            print('setting all values for dataset {0} to nan...'.format(dataset_name), flush=True)
            dataset['all'].matrix[:,dataset['all'].columnmeta['dataset'] == dataset_name] = np.nan
        ...
        for partition in ['all']:
            if np.isnan(dataset[partition].matrix).any():
                ...
                # compute a running stdv and mean
                dp = copy.deepcopy(dataset[partition])
                is_missing = np.isnan(dp.matrix)
                for i in range(random_imputation_iterations):
                ...
                recon[partition].matrix /= random_imputation_iterations
                embed[partition].matrix /= random_imputation_iterations
                error[partition] /= random_imputation_iterations
                if d['current_apply_activation_to_embedding']:
                    embed_preactivation[partition].matrix /= random_imputation_iterations
            else:
                if d['use_batchnorm']:
        ...
        for partition in partitions:
            ...
            if save_reconstructions:
                datasetIO.save_datamatrix('{0}/{1}_intermediate_reconstructions_layer{2!s}_finetuning{3!s}_step{4!s}.pickle'.format(d['output_path'], partition, d['current_hidden_layer'], d['current_finetuning_run'], selected_step), recon[partition])
                datasetIO.save_datamatrix('{0}/{1}_intermediate_reconstructions_layer{2!s}_finetuning{3!s}_step{4!s}.txt.gz'.format(d['output_path'], partition, d['current_hidden_layer'], d['current_finetuning_run'], selected_step), recon[partition])
            if d['current_apply_activation_to_embedding']:
        ...
        # plot heatmap
            ...
            parts = ['valid', 'test', 'valid_test']
            for partition in parts:
                if partition == 'all':
                    partsuff = ''
                else:
                    partsuff = '_' + partition
                embed[partition].cluster('all', 'cosine', 'average')
                embed[partition].heatmap(rowmetalabels=[], columnmetalabels=[], normalize=False, standardize=False, normalizebeforestandardize=True, cmap_name='bwr', ub=None, lb=None, savefilename='{0}/intermediate_embedding_heatmap_layer{1!s}_finetuning{2!s}_step{3!s}{4}.png'.format(d['output_path'], d['current_hidden_layer'], d['current_finetuning_run'], selected_step, partsuff), closefigure=True, dpi=300)
                if d['current_apply_activation_to_embedding']:
                    embed_preactivation['all'].cluster('all', 'cosine', 'average')
                    embed_preactivation['all'].heatmap(rowmetalabels=[], columnmetalabels=[], normalize=False, standardize=False, normalizebeforestandardize=True, cmap_name='bwr', ub=None, lb=None, savefilename='{0}/intermediate_embedding_preactivation_heatmap_layer{1!s}_finetuning{2!s}_step{3!s}{4}.png'.format(d['output_path'], d['current_hidden_layer'], d['current_finetuning_run'], selected_step, partsuff), closefigure=True, dpi=300)
                    

    print('done get_tsdae_features.', flush=True)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--random_imputation_iterations', help='number of times to compute embedding and reconstruction with randomly imputed NaNs', type=int, default=5)
    parser.add_argument('--save_reconstructions', help='whether or not to save reconstructed datamatrix', type=bool, default=False)
    parser.add_argument('--nan_datasets', help='datasets for which to nan all values before computing embedding and reconstruction', type=str, nargs='+')
    args = parser.parse_args()
    main(args.visualizations_path, args.random_imputation_iterations, args.save_reconstructions, args.nan_datasets)
