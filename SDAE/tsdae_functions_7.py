            ...
            for epoch in range(d['current_epochs']):
                ...
                for batch in range(d['batches']):
                    ...
                    if training_step == reporting_steps[i]:
                        train_losses[i] = sess.run(loss, feed_dict={training:False, selection_mask:is_train, noise_prob:0})
                        train_noisy_losses[i] = sess.run(loss, feed_dict={training:False, selection_mask:is_train, noise_prob:d['noise_probability']})
                        valid_losses[i] = sess.run(loss, feed_dict={training:False, selection_mask:is_valid, noise_prob:0})
                        valid_noisy_losses[i] = sess.run(loss, feed_dict={training:False, selection_mask:is_valid, noise_prob:d['noise_probability']})
