...
# load clusters
with open('gene_cluster_matrix.pickle', 'rb') as fr:
    gene_cluster = pickle.load(fr)

# select datasets
dataset_info = datasetselection.finddatasets(getalllevels=True)
# included_datasetabbrevs = {'biogrid', 'clinvar', 'cmap', 'ctdchemical', 'ctddisease_cleaned', 'dbgap_cleaned', 'depod', 'encodehm', 'encodetf', 'epigenomicshm', 'gad', 'gadhighlevel_cleaned', 'gobp', 'gocc', 'gomf', 'guidetopharmchemical', 'guidetopharmprotein', 'gwascatalog_cleaned', 'gwasdbdisease_cleaned', 'gwasdbphenotype_cleaned', 'hpo', 'hugenavigator', 'humancyc', 'intact', 'interpro', 'jasparpwm', 'kegg', 'lincscmapchemical', 'locate', 'locatepredicted', 'mgimpo', 'mirtarbase', 'nursa', 'omim', 'panther', 'reactome', 'targetscan_cleaned', 'targetscannonconserved_cleaned', 'virusmint', 'virusmintppi', 'wikipathways'}
...
#    if datasetabbrev not in {'kegg', 'panther', 'reactome', 'wikipathways'}:
#        continue
...
#    gene_atb.rowmeta['class'][np.in1d(gene_atb.rowlabels, class_examples['negative'])] = 'negative'
    ...
    gene_atb.columnmeta['arbitrary_pvalues'][~gene_atb.columnmeta['isrowstat']] = feature_selection_test_function(X=gene_atb.matrix[:,~gene_atb.columnmeta['isrowstat']], Y=gene_cst.select([],'0'))[1]
    gene_atb.columnmeta['arbitrary_pvalues'][gene_atb.columnmeta['isrowstat']] = featureselection.univariate_utest(X=gene_atb.matrix[:,gene_atb.columnmeta['isrowstat']], Y=gene_cst.select([],'0'))[1]
...
#                    gene_atb.matrix[:,i] = (gene_atb.matrix[:,mrg]*wgt[np.newaxis,:]).sum(1)
    ...
    for j, cluster_id in enumerate(gene_cst.columnlabels):
        if anynonstats:
            atb_cst['test_statistic_values'].matrix[~gene_atb.columnmeta['isrowstat'],j], atb_cst['pvalues'].matrix[~gene_atb.columnmeta['isrowstat'],j] = feature_selection_test_function(X=gene_atb.matrix[:,~gene_atb.columnmeta['isrowstat']], Y=gene_cst.matrix[:,j])
        if anyrowstats:
