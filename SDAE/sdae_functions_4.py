...
def create_autoencoder(x, activation, apply_activation_to_output, apply_activation_to_embedding, W, bencode, bdecode):
...
def main(d):
    ...
    h, hhat, xhat = create_autoencoder(xnoisy, activation_function['tf'], d['apply_activation_to_output'], d['current_apply_activation_to_embedding'], W, bencode, bdecode)
...
#        selected_step = reporting_steps[-1]
    ...
    x_valid = dataset['valid'].matrix[:d['reconstruction_rows']*d['reconstruction_cols'],:]
    xr_valid = recon['valid'].matrix[:d['reconstruction_rows']*d['reconstruction_cols'],:]
