...
'''
# tune learning rate
learning_rates = np.logspace(-4, -2, 5, dtype='float32')
...
epochs = 1000000
# reporting_steps = np.unique(np.logspace(0, np.log10(epochs*batches), reports_per_log*np.log10(epochs*batches)+1, dtype='int32'))
reporting_steps = np.concatenate((np.arange(1, 10, 1, dtype='int32'), np.arange(10, 100, 10, dtype='int32'), np.arange(100, 1000, 100, dtype='int32'), np.arange(1000, 10000, 1000, dtype='int32'), np.arange(10000, 100000, 10000, dtype='int32'), np.arange(100000, max(100000, epochs*batches)+100000, 100000, dtype='int32')))
reporting_steps = reporting_steps[reporting_steps < epochs*batches]
reporting_steps = np.append(reporting_steps, [epochs*batches])
print('reporting_steps', reporting_steps)
...
init = tf.global_variables_initializer()
sess.run(init)
restore_path = 'results/autoencoder/variables_layer{0!s}_finetuning{1!s}'.format(hiddenlayers, finetuning_run-1)
if os.path.exists(restore_path+'.meta'):
    saver.restore(sess, restore_path)
else:
    raise ValueError('no prior session found!')
...
for epoch in range(epochs):
    ...
    for batch in range(batches):
        ...
        if training_step == reporting_steps[i]:
            ...
            # if epoch >= 100 and valid_losses[i] >= valid_losses[:i].min():
            #     overfitting_score += 1
            # else:
            #     overfitting_score = 0
            # if epoch >= 1000 and overfitting_score == 5:
            #     stopearly = True
            #     print('stopping early!')
            #     break
            if training_step >= 100000:
                saver.save(sess, 'results/autoencoder/intermediate_variables_layer{0!s}_finetuning{1!s}_step{2!s}'.format(hiddenlayers, finetuning_run, training_step))
            # if epoch >= 990 and np.mod(i,9) == 0:
                # saver.save(sess, 'results/autoencoder/intermediate_variables_layer{0!s}_finetuning{1!s}'.format(hiddenlayers, finetuning_run))
...
#                    pickle.dump((sess.run(global_step), sess.run(W), sess.run(bencode), sess.run(bdecode)), fw)
        ...
        training_step += 1
if stopearly:
    saver.restore(sess, 'results/autoencoder/intermediate_variables_layer{0!s}_finetuning{1!s}'.format(hiddenlayers, finetuning_run))
...
with open('results/autoencoder/variables_layer{0!s}_finetuning{1!s}.pickle'.format(hiddenlayers, finetuning_run), 'wb') as fw:
    pickle.dump((sess.run(global_step), sess.run(W), sess.run(bencode), sess.run(bdecode)), fw)
saver.save(sess, 'results/autoencoder/variables_layer{0!s}_finetuning{1!s}'.format(hiddenlayers, finetuning_run))
