...
# selected maps
dataset_features = {}
with open('univariate_feature_importance_{0}/selected_interesting_features_missing_from_coexp.txt'.format(analysis_version), 'rt') as fr:
    fr.readline()
    for line in fr:
        dataset_name, dataset_abbreviation, significant_feature = [x.strip() for x in line.split('\t')]
        if dataset_abbreviation not in dataset_features:
            dataset_features[dataset_abbreviation] = []
        dataset_features[dataset_abbreviation].append(significant_feature)
...
excluded_datasetabbrevs = set(dataset_info.keys()).difference(dataset_features.keys())
...
#        gene_cst = pickle.load(fr)
    ...
    # keep only SELECTED significant features (include effect size cutoff)
    atb_cst['is_significant'].matrix = np.logical_and(atb_cst['is_significant'].matrix, atb_cst['test_statistic_values'].matrix > 0.01)
    tobediscarded = ~np.logical_and(atb_cst['is_significant'].matrix.any(1), np.in1d(atb_cst['is_significant'].rowlabels, dataset_features[datasetabbrev]))
    for layer in results_layers:
...
#            clb = -cub
            ...
            bmat = bbox.get_points()
            bmat[1,0] = bmat[0,0] + 0.125*(bmat[1,0]-bmat[0,0])
            bbox.set_points(bmat)
            ax.set_position(bbox)
            ax.imshow(np.linspace(clb, cub, 11, dtype='float64').reshape(-1,1), cmap=plt.get_cmap('jet'), alpha=1.0, aspect='auto', extent=(0, 1, clb, cub), origin='lower', interpolation='bicubic')
            ax.yaxis.set_label_position('right')
            ax.set_ylabel('-log10(q-value)', fontsize=8, fontname='arial', labelpad=3)
            ax.tick_params(axis='both', which='major', bottom='off', top='off', left='off', right='on', labelbottom='off', labeltop='off', labelleft='off', labelright='on', labelsize=8)
            ax.set_xlim(0, 1)
...
#            ax.axhline(yub, linewidth=0.5, color='k')
    ...
    if ia < num_axs:
        for j in range(ia, num_axs):
            plt.delaxes(axs[j])
    fg.savefig('univariate_feature_importance_{0}/{1}_selected_maps_heatmaps.png'.format(analysis_version, datasetabbrev), transparent=True, pad_inches=0, dpi=1200)
#    fg.show()
    plt.close()
print('done.')
