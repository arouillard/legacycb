...
#    gene_atb.rowmeta['class'][np.in1d(gene_atb.rowlabels, class_examples['negative'])] = 'negative'
    ...
    gene_atb.columnmeta['arbitrary_pvalues'][~gene_atb.columnmeta['isrowstat']] = featureselection.univariate_chisquare(X=gene_atb.matrix[:,~gene_atb.columnmeta['isrowstat']], Y=gene_cst.select([],'0'))[1]
...
#                    gene_atb.matrix[:,i] = (gene_atb.matrix[:,mrg]*wgt[np.newaxis,:]).sum(1)
    ...
    atb_cst['test_statistic_values'].matrix, atb_cst['pvalues'].matrix = feature_selection_test_function(X=gene_atb.matrix, Y=gene_cst.matrix, numperm=10**6)
