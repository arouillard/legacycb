...
def main(d):
    ...
    print('posweights min:', np.nanmin(posweights), flush=True)
    print('posweights max:', np.nanmax(posweights), flush=True)
    print('negweights nan:', np.isnan(negweights).any(), flush=True)
    print('negweights min:', np.nanmin(negweights), flush=True)
    print('negweights max:', np.nanmax(negweights), flush=True)
    ...
    print('marginalprobabilities min:', np.nanmin(marginalprobabilities), flush=True)
    print('marginalprobabilities max:', np.nanmax(marginalprobabilities), flush=True)
    print('marginalstdvs nan:', np.isnan(marginalstdvs).any(), flush=True)
    print('marginalstdvs min:', np.nanmin(marginalstdvs), flush=True)
    print('marginalstdvs max:', np.nanmax(marginalstdvs), flush=True)
    ...
    loss_weights = (pos_weights*is_positive + neg_weights*(one_-is_positive))*(one_-is_missing) # missing values won't be included in loss calculation
    ...
    noise_mask = tf.to_float(tf.random_uniform(tf.shape(x)) <= noise_prob) # replace missing values and random fraction of known values with noise
    ...
    normal_loss_mask = (one_-is_missing)*(one_-is_bernoulli_marginal)
    normal_loss = tf.reduce_sum(normal_loss_mask*tf.squared_difference(x, xhat_preactivation))/tf.reduce_sum(normal_loss_mask)
    bernoulli_loss_mask = (one_-is_missing)*is_bernoulli_marginal
    bernoulli_loss = tf.reduce_sum(bernoulli_loss_mask*tf.nn.sigmoid_cross_entropy_with_logits(labels=x, logits=xhat_preactivation))/tf.reduce_sum(bernoulli_loss_mask)
    loss = normal_loss + bernoulli_loss
    print('x.shape', x.shape, flush=True)
    print('is_positive.shape', is_positive.shape, flush=True)
    print('is_missing.shape', is_missing.shape, flush=True)
    print('loss_weights.shape', loss_weights.shape, flush=True)
    print('normal_noise.shape', normal_noise.shape, flush=True)
    print('bernoulli_noise.shape', bernoulli_noise.shape, flush=True)
    print('noise.shape', noise.shape, flush=True)
    print('noise_mask.shape', noise_mask.shape, flush=True)
    print('xnoisy.shape', xnoisy.shape, flush=True)
    print('normal_loss_mask.shape', normal_loss_mask.shape, flush=True)
    print('normal_loss.shape', normal_loss.shape, flush=True)
    print('bernoulli_loss_mask.shape', bernoulli_loss_mask.shape, flush=True)
    print('bernoulli_loss.shape', bernoulli_loss.shape, flush=True)
        ...
        with open('{0}/log_layer{1!s}_finetuning{2!s}.txt'.format(d['output_path'], d['current_hidden_layer'], d['current_finetuning_run']), mode='wt', buffering=1) as fl:
            ...
            for epoch in range(d['current_epochs']):
                ...
                for batch in range(d['batches']):
                    ...
                    # update weights
                    sess.run(train_ops, feed_dict={training:True, selection_mask:selected, noise_prob:d['noise_probability']})
                    
                    # record training and validation errors
                    if training_step == reporting_steps[i]:
                        train_losses[i], train_losses_normal[i], train_losses_bernoulli[i] = sess.run([loss, normal_loss, bernoulli_loss], feed_dict={training:False, selection_mask:is_train, noise_prob:0})
                        train_noisy_losses[i], train_noisy_losses_normal[i], train_noisy_losses_bernoulli[i] = sess.run([loss, normal_loss, bernoulli_loss], feed_dict={training:False, selection_mask:is_train, noise_prob:d['noise_probability']})
                        valid_losses[i], valid_losses_normal[i], valid_losses_bernoulli[i] = sess.run([loss, normal_loss, bernoulli_loss], feed_dict={training:False, selection_mask:is_valid, noise_prob:0})
                        valid_noisy_losses[i], valid_noisy_losses_normal[i], valid_noisy_losses_bernoulli[i] = sess.run([loss, normal_loss, bernoulli_loss], feed_dict={training:False, selection_mask:is_valid, noise_prob:d['noise_probability']})
                        print('total losses', flush=True)
                        ...
                        print('normal losses', flush=True)
                        print('step:{0:1.6g}, train loss:{1:1.3g}, valid loss:{2:1.3g}, train noisy loss:{3:1.3g},valid noisy loss:{4:1.3g}, time:{5:1.6g}'.format(reporting_steps[i], train_losses_normal[i], valid_losses_normal[i], train_noisy_losses_normal[i], valid_noisy_losses_normal[i], time.time() - starttime), flush=True)
                        print('bernoulli losses', flush=True)
                        print('step:{0:1.6g}, train loss:{1:1.3g}, valid loss:{2:1.3g}, train noisy loss:{3:1.3g},valid noisy loss:{4:1.3g}, time:{5:1.6g}'.format(reporting_steps[i], train_losses_bernoulli[i], valid_losses_bernoulli[i], train_noisy_losses_bernoulli[i], valid_noisy_losses_bernoulli[i], time.time() - starttime), flush=True)
