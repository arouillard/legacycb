...
def create_bopt_search_combinations(X, Y, domains, bopt_config, acquisition_types, model_types, model_estimation_methods, kernels):
    ...
    if model_estimation_method == 'MCMC':
...
def create_initial_hyperparameter_combinations(search_type, base_model_config, hyperparameters, domains, hyperparameter_specs):
    if search_type == 'line':
...
def append_search_data_arrays(model_config, model_config_path, search_type, combination_id, hyperparameters, X, Y, is_queued, search_types, combination_ids, job_ids, job_times, model_config_paths, model_configs):
    ...
    search_types = np.insert(search_types, search_types.size, 'random')
...
def main(search_config_path, job_config_path):
    ...
    if sc['y_transformation'] == 'neglog':
        transform_y = lambda y: -np.log10(y)
    elif sc['y_transformation'] == 'log':
        transform_y = lambda y: np.log10(y)
    elif sc['y_transformation'] == 'neg':
...
#    Xsd = np.zeros((0, len(hyperparameters)), dtype='float64')
...
#    combination_id = 0
    ...
    hyperparameter_combinations, hyperparameter_search_types = create_initial_hyperparameter_combinations(sc['search_type'], base_model_config, hyperparameters, domains, hyperparameter_specs)
    ...
    '''
    if sc['search_type'] == 'line':
        hyperparameter_combinations = create_line_search_combinations(base_model_config, hyperparameters, hyperparameter_specs)
        
        print('creating hyperparameter variations using line search...', flush=True)
        Xl = np.zeros((0, len(hyperparameters)), dtype='float64')
        Xbase = np.array([base_model_config['search_domain'][hp] for hp in hyperparameters], dtype='float64').reshape(1,-1)
        # iterate over hyperparameters
        for j, hp in enumerate(hyperparameters):
            Xi = Xbase.copy()
            ...
            for search_domain_value in hyperparameter_specs[hp]['grid']:
                Xi[0,j] = search_domain_value
                Xl = np.append(Xl, Xi.copy(), 0)
        # iterate over hyperparameters
        for hyperparameter, specs in hyperparameter_specs.items():
            print('working on hyperparameter {0}...'.format(hyperparameter), flush=True)
            mc = copy.deepcopy(base_model_config)
            
            # iterate over hyperparameter values
            for search_domain_value in specs['grid']:
                mc['search_domain'][hyperparameter] = search_domain_value
                mc['model_space'][hyperparameter] = specs['search_domain_to_model_space'](search_domain_value)
                print('updating {0} to {1!s}...'.format(hyperparameter, mc['model_space'][hyperparameter]), flush=True)
                
                # finish model_config (create folder and save model_config)
                print('finishing model_config...', flush=True)
                mc, mc_path = finish_model_config(mc, combination_id, search_folder)
                
                # update search data arrays
                print('updating search data arrays...', flush=True)
                X, Y, is_queued, search_types, combination_ids, job_ids, job_times, model_config_paths, model_configs = append_search_data_arrays(mc, mc_path, 'line', combination_id, hyperparameters, X, Y, is_queued, search_types, combination_ids, job_ids, job_times, model_config_paths, model_configs)
        
                # increment combination_id
                combination_id += 1
    
    elif sc['search_type'] == 'grid' or (sc['search_type'] == 'bopt' and sc['num_grid_points_per_hyperparameter'] > 0):
        print('creating hyperparameter variations using grid search...', flush=True)
        Xg = np.array(list(product(*[hyperparameter_specs[hp]['grid'] for hp in hyperparameters])), dtype='float64')
        np.random.shuffle(Xg)
        print('num_hyperparameters: {0!s}'.format(len(hyperparameters)), flush=True)
        print('num_grid_points_per_hyperparameter: {0!s}'.format(sc['num_grid_points_per_hyperparameter']), flush=True)
        print('num_grid_points: {0!s}'.format(Xg.shape[0]), flush=True)
        Xsd = np.append(Xsd, Xg, 0)
    
    if sc['search_type'] == 'bopt':
        print('creating hyperparameter variations using random search...', flush=True)
        print('num_initial_random_points: {0!s}'.format(sc['min_initial_points']), flush=True)
        Xr = np.zeros((sc['min_initial_points'], len(domains)), dtype='float64') 
        for j, hpd in enumerate(domains):
            if hpd['type'] == 'continuous':
                Xr[:,j] = (hpd['domain'][-1] - hpd['domain'][0])*np.random.rand(sc['min_initial_points']) + hpd['domain'][0]
            else:
                Xr[:,j] = np.random.choice(hpd['domain'], sc['min_initial_points'], replace=True)
        Xsd = np.append(Xr, Xsd, 0)
        
    # iterate over hyperparameter combinations
    for combination_values in Xsd:
        print('working on hyperparameter combination {0!s}...'.format(combination_id), flush=True)
        ...
        print('creating base model_config...', flush=True)
        print('working on hyperparameter combination {0!s}...'.format(combination_id), flush=True)
        mc = copy.deepcopy(base_model_config)
        
        # finish model_config
        print('finishing model_config...', flush=True)
#        mc_path = finish_model_config.main(mc)
        combination_folder = '{0}/hp_combination_{1!s}'.format(search_folder, combination_id)
        mc['search_domain']['save_folder'] = combination_folder
        mc['model_space']['save_folder'] = combination_folder
        if not os.path.exists(combination_folder):
            os.makedirs(combination_folder)
        print('combination_folder: {0}'.format(combination_folder), flush=True)
        print('combination_id: {0!s}'.format(combination_id), flush=True)
        
        # save model_config
        print('saving model_config...', flush=True)
        mc_path = '{0}/input.json'.format(mc['model_space']['save_folder'])
        print('mc_path: {0}'.format(mc_path), flush=True)
        with open(mc_path, mode='wt', encoding='utf-8', errors='surrogateescape') as fw:
            json.dump(mc['model_space'], fw, indent=2)
        
        # append search data to arrays
        print('appending search data to arrays...', flush=True)
        X['search_domain'] = np.append(X['search_domain'], np.array([mc['search_domain'][hp] for hp in hyperparameters], dtype='float64').reshape(1,-1), 0)
        X['model_space'] = np.append(X['model_space'], np.array([mc['model_space'][hp] for hp in hyperparameters], dtype='object').reshape(1,-1), 0)
        Y = np.insert(Y, Y.size, np.nan)
        is_queued = np.insert(is_queued, is_queued.size, True)
        search_types = np.insert(search_types, search_types.size, 'base')
        combination_ids = np.insert(combination_ids, combination_ids.size, combination_id)
        job_ids = np.insert(job_ids, job_ids.size, '')
        model_config_paths.append(mc_path)
