...
class datamatrix(object):
    ...
    def heatmap(self, rowmetalabels=None, columnmetalabels=None, normalize=False, standardize=False, normalizebeforestandardize=True, cmap_name='bwr', ub=None, lb=None, savefilename=None, closefigure=False, dpi=300):
        ...
        if columnmetalabels == None or len(columnmetalabels) == 0:
            ...
            ax.invert_yaxis()
            ax.set_yticklabels(columnmetalabels, minor=False, fontsize=8)
            ax.grid(False)
            ax.tick_params(axis='x', which='major', bottom=False, top=False, labelbottom=False, labeltop=False, pad=4)
            ax.tick_params(axis='y', which='major', left=False, right=False, labelleft=False, labelright=True, pad=4)
        if rowmetalabels == None or len(rowmetalabels) == 0:
            ...
            ax.invert_yaxis()
            ax.set_xticklabels(rowmetalabels, minor=False, rotation=90, fontsize=8)
            ax.grid(False)
            ax.tick_params(axis='x', which='major', bottom=False, top=False, labelbottom=True, labeltop=False, pad=4)
            ax.tick_params(axis='y', which='major', left=False, right=False, labelleft=False, labelright=False, pad=4)
        ...
        ax.invert_yaxis()
        ax.set_xticklabels(columnlabels, minor=False, rotation=90, fontsize=8*50/len(columnlabels))
        ax.set_yticklabels(rowlabels, minor=False, fontsize=8*30/len(rowlabels))
        ax.grid(False)
        ax.tick_params(axis='x', which='major', bottom=False, top=False, labelbottom=True, labeltop=False, pad=4)
        ax.tick_params(axis='y', which='major', left=False, right=False, labelleft=False, labelright=True, pad=4)
        ax.set_ylabel(rowname, fontsize=8, labelpad=2)
        ax.set_title(columnname, loc='center', fontsize=8)
