...
def submit_job(job_config, model_config_path):
    ...
    print('job_command: {0}'.format(job_command), flush=False)
...
def run_job(job_runner, model_config_path, datamatrix, combination_id):
    job_runner.main(model_config_path, datamatrix)
...
def create_line_search_combinations(base_model_config, hyperparameters, hyperparameter_specs):
    ...
    print('creating hyperparameter combinations using line search...', flush=False)
    ...
    print('num_hyperparameter_combinations: {0!s}'.format(hyperparameter_combinations.shape[0]), flush=False)
...
def create_grid_search_combinations(hyperparameters, hyperparameter_specs):
    ...
    print('creating hyperparameter combinations using grid search...', flush=False)
...
def create_random_search_combinations(num_combinations, domains):
    ...
    print('creating hyperparameter combinations using random search...', flush=False)
...
def create_bopt_search_combinations(X, Y, domains, bopt_config, acquisition_types, model_types, model_estimation_methods, kernels):
    ...
    print('creating hyperparameter combinations using bayesian optimization...', flush=False)
    ...
    print('acquisition type: {0}'.format(bopt_config['acquisition_type']), flush=False)
    print('model type: {0}'.format(bopt_config['model_type']), flush=False)
    print('kernel: {0}'.format(kernel_name), flush=False)
    print('fitting {0} {1} model and finding {2} optimum...'.format(bopt_config['model_type'], kernel_name, bopt_config['acquisition_type']), flush=False)
    ...
    print(bopt.model.model, flush=False)
    print('bopt iteration time: {0:1.3g} min'.format((time.time() - bopt_start_time)/60.0), flush=False)
    print('num_hyperparameter_combinations: {0!s}'.format(hyperparameter_combinations.shape[0]), flush=False)
    return hyperparameter_combinations, search_types
...
def create_model_config(combination_values, hyperparameters, hyperparameter_specs, base_model_config):
    ...
    print('copying base_model_config...', flush=False)
    ...
    for hyperparameter, search_domain_value in zip(hyperparameters, combination_values):
        ...
        print('updating {0} to {1!s}...'.format(hyperparameter, model_config['model_space'][hyperparameter]), flush=False)
    return model_config

def save_model_config(model_config, combination_id, search_folder):
    # create save_folder
    print('creating save_folder...', flush=False)
    ...
    print("combination_folder / model_config['save_folder']: {0}".format(combination_folder), flush=False)
    # save model_config
    print('saving model_config...', flush=False)
...
def append_search_data_arrays(model_config, model_config_path, search_type, combination_id, hyperparameters, X, Y, is_queued, is_completed, is_success, search_types, combination_ids, job_ids, job_run_times, job_start_times, num_completed_combinations_at_job_start, model_config_paths, model_configs):
    ...
    print('appending hyperparameter combination info to search data arrays...', flush=False)
...
def save_search_data(search_data_path, X, Y, is_queued, is_completed, is_success, search_types, combination_ids, job_ids, job_run_times, job_start_times, num_completed_combinations_at_job_start, model_config_paths, model_configs, base_model_config, hyperparameter_specs, hyperparameters, domains):
    # save search data
    print('writing to {0}...'.format(search_data_path), flush=False)
...
#        pickle.dump({'X':X, 'Y':Y, 'is_queued':is_queued, 'is_completed':is_completed, 'is_success':is_success, 'search_types':search_types, 'combination_ids':combination_ids, 'job_ids':job_ids, 'job_run_times':job_run_times, 'job_start_times':job_start_times, 'num_completed_combinations_at_job_start':num_completed_combinations_at_job_start, 'model_config_paths':model_config_paths, 'model_configs':model_configs, 'base_model_config':base_model_config, 'hyperparameter_specs':hyperparameter_specs, 'hyperparameters':hyperparameters, 'domains':domains}, fw)
    ...
    print('writing to {0}...'.format(search_data_path.replace('pickle', 'txt')), flush=False)
...
def main(search_config_path, job_config_path, model_config_path, search_id='max'):
    ...
    print('loading search configuration...', flush=False)
    ...
    print('loading job configuration...', flush=False)
    print('job_config_path: {0}'.format(job_config_path), flush=False)
    jc = load_config(job_config_path)


    # load job runner
    print('loading job runner...', flush=False)
    jc['script_folder'], jc['script_file'] = jc['script_path'].rsplit('/', maxsplit=1)
    sys.path.append(jc['script_folder'])
    job_runner = __import__(jc['script_file'].replace('.py',''))

    
    # load model configuration
    print('loading model configuration...', flush=False)
    print('model_config_path: {0}'.format(model_config_path), flush=False)
    ...
    print('datamatrix_path: {0}'.format(mc['datamatrix_path']), flush=False)
    datamatrix = datasetIO.load_datamatrix(mc['datamatrix_path'])

    
    # set folders
    print('setting folders...', flush=False)
    ...
    print('current search_folder: {0}'.format(search_folder), flush=False)
    print('current search_id: {0!s}'.format(search_id), flush=False)
    
    
    # create base model_config and collect hyperparameter search specs
    print('creating base model_config and collecting hyperparameter search specs...', flush=False)
    ...
    # create initial hyperparameter combinations
    print('creating initial hyperparameter combinations...', flush=False)
    ...
    for combination_id, (combination_values, search_type) in enumerate(zip(hyperparameter_combinations, hyperparameter_search_types)):
        print('working on hyperparameter combination {0!s}...'.format(combination_id), flush=False)
        
        # create model_config
        print('creating model_config...', flush=False)
    ...
    print('saving initial search data...', flush=False)
    ...
    # run search
    print('running search...', flush=False)
    ...
    while (elapsed_time < sc['search_time'] and num_suggestions < sc['max_suggestions'] and num_consecutive_bopt_failures < max_consecutive_bopt_failures and (sc['search_type'] == 'bopt' or sc['search_type'] == 'random')) or is_queued.any():
        ...
        print('ACTIVE COMBINATIONS: {0!s}'.format(num_active_combinations), flush=False)
        ...
        print('COMPLETED COMBINATIONS: {0!s}'.format(num_completed_combinations), flush=False)
        num_successful_combinations = is_success.sum()
        print('SUCCESSFUL COMBINATIONS: {0!s}'.format(num_successful_combinations), flush=False)
        ...
        if num_successful_combinations > prev_num_successful_combinations:
            ...
            print('MINIMUM LOSS: {0!s}'.format(Y[i_min]), flush=False)
            ...
            print({k:v for k,v in zip(z_headers, Z[-1])}, flush=False)
        ...
        if num_active_combinations >= sc['max_active_points']:
            print('REACHED MAX ACTIVE COMBINATIONS: {0!s}. WAITING...'.format(sc['max_active_points']), flush=False)
            time.sleep(59)
...
#            print('SUBMITTING QUEUED GRID OR LINE COMBINATION {0!s}...'.format(combination_id), flush=False)
#            job_id = submit_job(jc, model_config_paths[combination_id])
            print('RUNNING QUEUED GRID OR LINE COMBINATION {0!s}...'.format(combination_id), flush=False)
            job_id = run_job(job_runner, model_config_paths[combination_id], copy.deepcopy(datamatrix), combination_id)
            ...
            if suggest_random_point or num_successful_combinations < sc['min_initial_points']:
                ...
                try:
                    ...
                    print('BOPT ERROR: {0} {1}'.format(bopt_error[0], bopt_error[1]), flush=False)
                    print('num_consecutive_bopt_failures: {0!s}'.format(num_consecutive_bopt_failures), flush=False)                    
                    hyperparameter_combinations = np.zeros((0, len(hyperparameters)), dtype='float64')
...
#                print('SUBMITTING NEW COMBINATION {0!s}...'.format(combination_id), flush=False)
#                job_id = submit_job(jc, model_config_paths[combination_id])
                print('RUNNING NEW COMBINATION {0!s}...'.format(combination_id), flush=False)
                job_id = run_job(job_runner, model_config_paths[combination_id], copy.deepcopy(datamatrix), combination_id)
                is_queued, job_ids, job_run_times, job_start_times, num_completed_combinations_at_job_start = update_job_info(job_id, combination_id, num_completed_combinations, is_queued, job_ids, job_run_times, job_start_times, num_completed_combinations_at_job_start)
                num_suggestions += 1
        
            # save search data
            print('saving search data...', flush=False)
            save_search_data(search_data_path, X, Y, is_queued, is_completed, is_success, search_types, combination_ids, job_ids, job_run_times, job_start_times, num_completed_combinations_at_job_start, model_config_paths, model_configs, base_model_config, hyperparameter_specs, hyperparameters, domains)
    ...
    while num_active_combinations > 0 and elapsed_time < (sc['search_time'] + 1 - 600/3600):
        print('ACTIVE COMBINATIONS: {0!s}. WAITING FOR JOBS TO FINISH...'.format(num_active_combinations), flush=False)
        time.sleep(300)
        num_active_combinations = count_active_jobs(job_ids)
        elapsed_time = (time.time() - start_time)/3600.0
    ...
    print('collecting final results...', flush=False)
    Y, is_completed, is_success, job_run_times = update_y(Y, transform_y, sc['y_failure_value'], is_queued, is_completed, is_success, job_ids, job_run_times, model_configs)
