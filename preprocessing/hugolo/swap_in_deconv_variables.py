...
dataset = datasetIO.load_datamatrix('../../original_data/hugolo_symlnk/patient_gene_hugolo_rnaseq_prepared_plus_clinical.pickle')
dataset.reorder(np.argsort(dataset.rowlabels), 0)
...
deconv_methods = ['epic', 'MCPCounter', 'quantiseq', 'timer', 'xcell']
for deconv_method in deconv_methods:
    print('working on deconv_method: {0}...'.format(deconv_method), flush=True)
    df = pd.read_csv('../../original_data/hugolo_symlnk/deconvoluted/deconvoluted_{0}.csv'.format(deconv_method), index_col=False)
    df.sort_values(by='Unnamed: 0', inplace=True)
    if df.shape[0] == dataset.shape[0] and (df['Unnamed: 0'].values == dataset.rowlabels).all():
...
datasetIO.save_datamatrix('../../original_data/hugolo_symlnk/patient_ft_hugolo_only_clinical_and_deconv.pickle', dataset)
datasetIO.save_datamatrix('../../original_data/hugolo_symlnk/patient_ft_hugolo_only_clinical_and_deconv.txt.gz', dataset)
savefolder = '../../input_data/hugolo_clinical_and_deconv'
if not os.path.exists(savefolder):
	os.makedirs(savefolder)
datasetIO.save_splitdata(savefolder, dataset)
shutil.copyfile('../../original_data/hugolo_symlnk/patient_ft_hugolo_only_clinical_and_deconv.pickle', '{0}/datamatrix.pickle'.format(savefolder))
shutil.copyfile('../../original_data/hugolo_symlnk/patient_ft_hugolo_only_clinical_and_deconv.txt.gz', '{0}/datamatrix.txt.gz'.format(savefolder))
