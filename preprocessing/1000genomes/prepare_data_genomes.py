...
# add hla types
print('adding hla types...', flush=True)
columnlabel_idx = {l:i for i,l in enumerate(snp_genome.columnlabels)}
...
for metalabel in hla_types_df.columns.values[1:]:
    snp_genome.columnmeta[metalabel] = np.full(snp_genome.shape[1], 'NA', dtype='object')
    ...
    uvals, counts = np.unique(snp_genome.columnmeta[metalabel], return_counts=True)
    max_num_uvals = 25
    if uvals.size > max_num_uvals:
        si = np.argsort(counts)[::-1]
        low_freq_uvals = uvals[si[max_num_uvals:]]
        snp_genome.columnmeta[metalabel][np.in1d(snp_genome.columnmeta[metalabel], low_freq_uvals)] = 'NA'
...
plt.close()
for metalabel in snp_genome.columnmeta: # ['population', 'super_population', 'gender']:
    ...
    for category, color in zip(categories, colors):
        ...
        ax.plot(pca_matrix[hit,0], pca_matrix[hit,1], linestyle='None', linewidth=0, marker='o', markerfacecolor=color, markeredgecolor=color, markersize=2, markeredgewidth=0, alpha=alpha, zorder=zorder, label=category)
    ax.tick_params(axis='both', which='major', bottom=False, top=False, labelbottom=False, labeltop=False, left=False, right=False, labelleft=False, labelright=False)
    ax.legend(loc='center left', bbox_to_anchor=(1, 0.5), borderaxespad=0, frameon=False, ncol=1, numpoints=1, markerscale=2, fontsize=8, labelspacing=0.25)
    ax.set_title('expl_var_frac: {0:1.3g}'.format(pca_model.explained_variance_ratio_.sum()), fontsize=8)
    ax.set_frame_on(False)
