...
def create_and_save_partitions(dataset, study_name, meta_label, test_groups, pretest_groups, valid_groups, save_text_files=True):
    ...
    tobepopped = ~dataset.rowmeta['is_core_sample']
    dataset_other = dataset.pop(tobepopped, 0)
    print('    OTHER', flush=True)
    print(dataset_other, flush=True)
    ...
    datasetIO.save_datamatrix('{0}/other.pickle'.format(savefolder), dataset_other)
    ...
    if save_text_files:
        ...
        os.mkdir('{0}/other'.format(savefolder))
        datasetIO.save_splitdata('{0}/other'.format(savefolder), dataset_other)
...
#        datasetIO.save_datamatrix('{0}/other.txt.gz'.format(savefolder), dataset_other)
...
def main(study_name, meta_label, test_groups, pretest_groups, valid_groups, row_data_path=None, column_data_path=None, matrix_data_path=None, partition_axis=0, dtype='float64', delimiter='\t', save_text_files=True):
    ...
    if row_data_path==None or column_data_path==None or matrix_data_path==None:
        ...
        if os.path.exists(loadfolder):
            ...
            dmpicklehit = ['datamatrix' in x and '.pickle' in x for x in original_files]
            dmtexthit = ['datamatrix' in x and ('.txt' in x or '.csv' in x) for x in original_files]
            if sum(dmpicklehit) > 0:
                datamatrix_path = '{0}/{1}'.format(loadfolder, original_files[dmpicklehit.index(True)])
                print('LOADING DATA...', flush=True)
                print('    datamatrix_path: {0}'.format(datamatrix_path), flush=True)
                dataset = datasetIO.load_datamatrix(datamatrix_path)
            elif sum(dmtexthit) > 0:
                datamatrix_path = '{0}/{1}'.format(loadfolder, original_files[dmtexthit.index(True)])
                print('LOADING DATA...', flush=True)
                print('    datamatrix_path: {0}'.format(datamatrix_path), flush=True)
                dataset = datasetIO.load_datamatrix(datamatrix_path)
            elif sum(rowhit) > 0 and sum(columnhit) > 0 and sum(matrixhit) > 0:
