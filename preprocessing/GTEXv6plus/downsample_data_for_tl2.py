...
with open('../../original_data/GTEXv6plus/GTEx_Data_V6_Annotations_SubjectPhenotypesDS.txt', mode='rt', encoding='utf-8', errors='surrogateescape') as fr:
    fr.readline()
    for line in fr:
...
with open('../../original_data/GTEXv6plus/GTEx_Data_V6_Annotations_SampleAttributesDS.txt', mode='rt', encoding='utf-8', errors='surrogateescape') as fr:
    fr.readline()
    for line in fr:
        entries = [x.strip() for x in line.split('\t')]
...
with open('../../original_data/GTEXv6plus/SRP012682.tsv', mode='rt', encoding='utf-8', errors='surrogateescape') as fr:
    fr.readline()
    for line in fr:
        entries = [x.strip() for x in line.split('\t')]
...
with gzip.open('../../original_data/GTEXv6plus/counts_gene.tsv.gz', mode='rt', encoding='utf-8', errors='surrogateescape') as fr:
...
selected_subjects = u_subject_ids[np.logical_and(c_subject_ids > 0.1*selected_specific_tissues.size, c_subject_ids < selected_specific_tissues.size)]
...
'''
with open('../../original_data/GTEXv6plus/selected_sample_counts_by_specific_tissue.txt', mode='wt', encoding='utf-8', errors='surrogateescape') as f1,\
     open('../../original_data/GTEXv6plus/selected_sample_counts_by_specific_tissue_and_sex.txt', mode='wt', encoding='utf-8', errors='surrogateescape') as f2,\
     open('../../original_data/GTEXv6plus/selected_sample_counts_by_specific_tissue_and_sex_and_coarse_age.txt', mode='wt', encoding='utf-8', errors='surrogateescape') as f3:
...
core_samples = np.empty(0, dtype='object')
with open('../../original_data/GTEXv6plus/sampling_scheme.txt', mode='rt', encoding='utf-8', errors='surrogateescape') as fr:
    fr.readline()
    for line in fr:
        ...
        if int(sample_selected) > 0:
            chosen_samples = np.append(chosen_samples, candidate_samples)
            core_samples = np.append(core_samples, np.random.choice(candidate_samples, int(sample_selected), replace=False))
with open('../../original_data/GTEXv6plus/chosen_samples_for_tl2.txt', mode='wt', encoding='utf-8', errors='surrogateescape') as fw:
    fw.write('\n'.join(chosen_samples))
with open('../../original_data/GTEXv6plus/core_samples_for_tl2.txt', mode='wt', encoding='utf-8', errors='surrogateescape') as fw:
    fw.write('\n'.join(core_samples))

hit = np.logical_and(np.in1d(sample_metadata['sample_id'], chosen_samples), sample_metadata['general_tissue'] != '')
sample_metadata['is_core_sample'] = np.in1d(sample_metadata['sample_id'], core_samples)
for field, values in sample_metadata.items():
    sample_metadata[field] = values[hit]
run_ids = run_ids[hit]

matrix = matrix = np.loadtxt('../../original_data/GTEXv6plus/counts_gene.tsv.gz', dtype='float64', delimiter='\t',
...
datasetIO.save_datamatrix('../../original_data/GTEXv6plus/gene_tissue_recount2gtexv6_chosen_samples_for_tl2_counts.pickle', gene_tissue)
datasetIO.save_datamatrix('../../original_data/GTEXv6plus/gene_tissue_recount2gtexv6_chosen_samples_for_tl2_counts.txt.gz', gene_tissue)
